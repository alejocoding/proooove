...
<script>
  window.addEventListener("DOMContentLoaded", () => {
    ...
    const editKilometraje = document.getElementById("editKilometraje");
    ...
    const anioAgregar = document.getElementById("anioAgregar")
    const modeloAgregar = document.getElementById("modeloAgregar")

    function validarLongitud(longitud, elemento) {
      const longitudDeElemento = elemento.value;

      if (longitudDeElemento.length > longitud) {
        elemento.value = longitudDeElemento.substring(0, longitud);
      }
    }

    editModelo.addEventListener("input", function() {
      validarLongitud(50, editModelo);
    });

    editAnio.addEventListener("input", function() {
      validarLongitud(4, editAnio);
    });
    
    editKilometraje.addEventListener("input", function() {
      validarLongitud(10, editKilometraje);
    });
    // -------------------------------------------------
    kilometrajeAgregar.addEventListener("input", function() {
      validarLongitud(10, kilometrajeAgregar);
    });

placaAgregar.addEventListener("input", function() {
  ...
  const validacionPlaca = document.getElementById('validacion_placa');
  ...
  if (this.value.length > 0) {
    if (placaRegex.test(this.value)) {
      ...
      validacionPlaca.style.opacity = '0';
    } else {
      this.classList.remove('is-valid');
      this.classList.add('is-invalid');
      validacionPlaca.style.opacity = '1';
    }
  ...
  validarLongitud(6, placaAgregar);
});

    anioAgregar.addEventListener("input", function() {
      validarLongitud(4, anioAgregar);
    });

    modeloAgregar.addEventListener("input", function() {
      validarLongitud(50, modeloAgregar);
    });
  })
</script>