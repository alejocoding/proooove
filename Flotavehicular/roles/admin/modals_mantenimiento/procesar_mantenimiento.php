...
try {
    ...
    switch ($accion) {
        case 'agregar':
            ...
            // Validaciones de coherencia adicionales
            $errores = [];

            // Validar fechas (formato y orden lógico)
            if (!empty($fecha_programada) && !empty($fecha_realizada) && $fecha_programada > $fecha_realizada) {
                $errores[] = 'La fecha realizada no puede ser anterior a la programada.';
            }

            if (!empty($fecha_realizada) && !empty($proximo_cambio_fecha) && $fecha_realizada > $proximo_cambio_fecha) {
                $errores[] = 'La fecha del próximo cambio no puede ser anterior a la realizada.';
            }

            // Validar kilometrajes
            if (!is_null($kilometraje_actual) && $kilometraje_actual < 0) {
                $errores[] = 'El kilometraje actual debe ser un número positivo.';
            }

            if (!is_null($kilometraje_actual) && !is_null($proximo_cambio_km) && $proximo_cambio_km <= $kilometraje_actual) {
                $errores[] = 'El kilometraje del próximo cambio debe ser mayor al actual.';
            }

            // Si hay errores, responder y detener el flujo
            if (!empty($errores)) {
                echo json_encode([
                    'success' => false,
