...
const inputs = document.querySelectorAll('#formtecnico input, #formtecnico select');
...
const validarformulario = (e) => {
    switch (e.target.name) {
        ...
        case "centro":
            validarSelect(e.target, 'grupo_centro', 'validacion3');
            break;
...
formulario.addEventListener('submit', function (e) {
    ...
    const validcentro = validarSelect(document.getElementById('centro'), 'grupo_centro', 'validacion3');
    ...
    if (validPlaca && validcentro && validFechaex && validFechaven && validestado) {
        const formdatos = new FormData(formulario)
        
        $.ajax({
            type: "POST",
            url: "../AJAX/guardar_tecno.php",
            data: formdatos,
            contentType: false,
            processData: false,
             success: function(response) {
                ...
                if (response.status === "success") {
                    ...
                    setTimeout(() => {
                        window.location.href = '../historiales/ver_tecnomecanica.php';
                    }, 3000);
                } else {
